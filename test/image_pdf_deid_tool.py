...
import pytesseract
# Windows 上若 tesseract 沒有加進 PATH，需要手動指定執行檔位置
pytesseract.pytesseract.tesseract_cmd = r"C:\Program Files\Tesseract-OCR\tesseract.exe"
try:
    import pymupdf as fitz  # 新版套件名稱，import fitz 已被標為 deprecated
except ImportError:
...
# Presidio 目前分析語言固定是英文 (language="en")，遇到中文內容常會誤判 (例如把中文字串
# 誤認成 US_DRIVER_LICENSE)，所以規則是：
#   1) 這段文字裡如果含有中文字，就整段跳過 Presidio，只跑上面的正則規則
#   2) 不論中英文，以下標籤一律不遮蔽 (誤判率太高、對這個用途沒意義)
IGNORED_PRESIDIO_LABELS = {"US_DRIVER_LICENSE"}
...
def detect_matches(text: str) -> List[Tuple[int, int, str]]:
    ...
    # 中文內容不跑 Presidio (它只支援英文分析，對中文常常誤判)
    if _analyzer is not None and not _contains_chinese(text):
        try:
            for r in _analyzer.analyze(text=text, language="en"):
                if r.entity_type in IGNORED_PRESIDIO_LABELS:
                    continue
...
def _run_interactive():
    """沒有帶命令列參數時 (例如在編輯器裡直接按「執行」)，改成用問答方式輸入，
    避免使用者只看到 argparse 的 usage 錯誤訊息就不知道怎麼用"""
    print("未偵測到命令列參數，改用互動模式。若要改用指令列方式執行，可參考檔案最上方的「用法範例」。\n")
    options = list(DISPATCH.keys())
    for i, name in enumerate(options, 1):
        print(f"  {i}. {name}")
    choice = input(f"請選擇要執行的功能 (1-{len(options)})：").strip()
    try:
        cmd = options[int(choice) - 1]
    except (ValueError, IndexError):
        print("輸入無效，結束。")
        return

    input_path = input("輸入檔案路徑：").strip().strip('"')
    output_path = input("輸出檔案路徑：").strip().strip('"')
    map_file = input(f"對照表檔案路徑 (直接按 Enter 使用預設 {DEFAULT_MAP_FILE})：").strip().strip('"')
    map_file = map_file or DEFAULT_MAP_FILE

    DISPATCH[cmd](input_path, output_path, map_file)
